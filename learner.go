...
func newLearner(id int, nt network, accrptors ...int) *learner {
	l := &learner{id: id, nt: nt, acceptors: make(map[int]accept)}
	for _, a := range accrptors {
...
func (l *learner) receiveAccepted(accepted message) {
	...
	if a.proposalNumber() < accepted.num {
...
// A proposal is chosen when it has been accepted by a majority of the acceptors.
// The leader might choose multiple proposals when it learns multiple times,
// but we guarante that all chosen proposals have the same value.
